...
public class MainActivity extends AppCompatActivity implements View.OnTouchListener, SeekBar.OnSeekBarChangeListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mSeekbar5.setProgress(15);
        mSeekbar6.setProgress(15);
        mSeekbar7.setProgress(15);
    ...
    public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
        switch (seekBar.getId()) {
            case R.id.seekbar1:
                mCobWebView.setPointNum(progress + 1);
                mTextView1.setText(String.format("小点数量： %d", mCobWebView.getPointNum()));
                break;
            case R.id.seekbar2:
                mCobWebView.setAcceleration(progress + 3);
                mTextView2.setText(String.format("加速度： %d", mCobWebView.getAcceleration()));
                break;
            case R.id.seekbar3:
                mCobWebView.setMaxDistance(progress + 20);
                mTextView3.setText(String.format("最大连线距离： %d", mCobWebView.getMaxDistance()));
                break;
            case R.id.seekbar4:
                mCobWebView.setLineAlpha(progress);
                mTextView4.setText(String.format("连线透明度： %d", mCobWebView.getLineAlpha()));
                break;
            case R.id.seekbar5:
                mCobWebView.setLineWidth(progress);
                mTextView5.setText(String.format("连线粗细： %d", mCobWebView.getLineWidth()));
                break;
            case R.id.seekbar6:
                mCobWebView.setPointRadius(progress);
                mTextView6.setText(String.format("小点半径： %d", mCobWebView.getPointRadius()));
                break;
            case R.id.seekbar7:
                mCobWebView.setTouchPointRadius(progress);
                mTextView7.setText(String.format("触摸点半径： %d", mCobWebView.getTouchPointRadius()));
                break;
            case R.id.seekbar8:
                mCobWebView.setPullBackRange(progress);
                mTextView8.setText(String.format("引力强度： %d", mCobWebView.getPullBackRange()));
                break;
        }
    }
